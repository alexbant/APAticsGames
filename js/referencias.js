document.addEventListener('DOMContentLoaded', () => {
  ...
  if (progressContainer) {
    ...
    const stars = progreso.referencias.estrellas || {};
    let totalStars = 0;
    for (let i = 1; i <= 5; i++) {
    ...
    let msgDiv = document.getElementById('ref-status-msg');
    if (!msgDiv) {
      msgDiv = document.createElement('div');
      msgDiv.id = 'ref-status-msg';
      ...
      msgDiv.innerHTML = `
        <div class="sidebar-msg success">
          ...
          <div>
            ...
            <p>Has dominado las Referencias APA con puntuación perfecta.</p>
          </div>
        </div>
      `;
    } else {
  ...
  const currentLevel = progreso.referencias.nivelActual || 1;

  cards.forEach((card, index) => {
    const levelNum = index + 1;
    const btn = card.querySelector('button') || card.querySelector('.btn-play');
    const lockedMsg = card.querySelector('.locked-msg');
    const targetUrl = `nivel${levelNum}A.html`; // URL específica para Referencias (Sufijo A)
    ...
    if (levelNum <= currentLevel) {
      ...
      if (btn) {
        ...
        btn.style.display = ''; // Restaurar display original (o inline-block)
        btn.style.pointerEvents = 'auto';
        btn.style.opacity = '1';
        btn.textContent = "Jugar";
        
        // Manejo robusto de enlace vs botón
        if (btn.tagName === 'A') {
          btn.href = targetUrl;
          btn.onclick = null; // Limpiar bloqueos previos
        } else {
          btn.disabled = false;
      ...
      // Mensaje de bloqueo (Arreglo visual del icono)
      if (lockedMsg) {
        lockedMsg.style.display = 'block';
        lockedMsg.innerHTML = '<i class="fa-solid fa-square-xmark"></i> Completa el nivel anterior';
      } else {
    ...
    // 2. Mostrar Estrellas Ganadas (Si tiene)
    const starsEarned = progreso.referencias.estrellas[levelNum] || 0;
    if (starsEarned > 0) {
