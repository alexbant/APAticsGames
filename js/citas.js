document.addEventListener('DOMContentLoaded', () => {
  ...
  if (progressContainer) {
    ...
    const stars = (progreso.citas && progreso.citas.estrellas) ? progreso.citas.estrellas : {};
    let totalStars = 0;
    for (let i = 1; i <= 5; i++) {
    ...
    let msgDiv = document.getElementById('citas-status-msg');
    if (!msgDiv) {
      msgDiv = document.createElement('div');
      msgDiv.id = 'citas-status-msg';
      ...
      msgDiv.innerHTML = `
        <div class="sidebar-msg success">
          ...
          <div>
            ...
            <p>Has dominado las Citas APA con puntuación perfecta.</p>
          </div>
        </div>
      `;
    } else {
  ...
  const currentLevel = (progreso.citas && progreso.citas.nivelActual) ? progreso.citas.nivelActual : 1;

  cards.forEach((card, index) => {
    const levelNum = index + 1;
    const btn = card.querySelector('button') || card.querySelector('.btn-play');
    const lockedMsg = card.querySelector('.locked-msg');
    const targetUrl = `nivel${levelNum}.html`; // URL específica para Citas
    ...
    const starsEarned = (progreso.citas && progreso.citas.estrellas && progreso.citas.estrellas[levelNum]) ? progreso.citas.estrellas[levelNum] : 0;
    if (starsEarned > 0) {
